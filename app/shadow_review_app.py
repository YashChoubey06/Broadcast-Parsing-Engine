...
def main():
    ...
    event = ParsedTradeEvent.from_json(pred_json)
    ...
    with st.expander("Manual Correction Form", expanded=False):
        c_action = st.selectbox("Action", ["OPEN_LONG", "OPEN_SHORT", "ADD_LONG", "ADD_SHORT", "REDUCE_POSITION", "CLOSE_POSITION", "HOLD_POSITION", "UPDATE_STOP_LOSS", "TARGET_HIT", "NON_TRADE", "AMBIGUOUS", "CONDITIONAL_INSTRUCTION", "CANCEL_PREVIOUS"], index=["OPEN_LONG", "OPEN_SHORT", "ADD_LONG", "ADD_SHORT", "REDUCE_POSITION", "CLOSE_POSITION", "HOLD_POSITION", "UPDATE_STOP_LOSS", "TARGET_HIT", "NON_TRADE", "AMBIGUOUS", "CONDITIONAL_INSTRUCTION", "CANCEL_PREVIOUS"].index(event.final_action) if event.final_action else 0)
        c_symbol = st.text_input("Symbol", event.symbol or "")
        c_dir = st.selectbox("Direction", ["LONG", "SHORT", "UNKNOWN"], index=["LONG", "SHORT", "UNKNOWN"].index(event.direction) if event.direction else 2)
        ...
        if st.button("Apply Manual Correction"):
            try:
                new_event = ParsedTradeEvent.from_json(event.to_json())
                ...
                execute_review_action("edit_and_approve", msg_id, corrected_event=new_event, changed_fields=changed_fields, notes=notes)
            except Exception as e:
