...
ACTION_TO_STATUS = {
    "Confirm Ordered Reversal": review.CONFIRMED_ORDERED_REVERSAL,
...
def _save(candidate, status, reviewer, notes, use_for_training, corrected):
    try:
...
def main():
    ...
    with st.sidebar:
        ...
        use_for_training_filter = st.selectbox("use_for_training", ["ALL", "true", "false"])
    ...
    metric_cols[3].metric("Training eligible", summary["training_eligible_records"])
    ...
    table_rows = [
        {
            ...
            "use_for_training": row.get("current_use_for_training", ""),
        }
        for row in filtered
    ]
    ...
    use_for_training = st.checkbox(
        "Use for training",
        value=current_decision.get("use_for_training") == "true",
        help="Only confirmed ordered reversals are training-compatible in this Phase 4 workflow.",
    )
    ...
    for col, label in zip(action_cols[:6], ACTION_TO_STATUS):
        with col:
            if st.button(label):
                _save(candidate, ACTION_TO_STATUS[label], reviewer, notes, use_for_training, corrected={})
    with action_cols[6]:
